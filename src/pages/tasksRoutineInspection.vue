      ...
      <div class="li_xunjian_title" style="height:auto">
        ...
        <div class="all_content">
          <el-radio v-model="radio" v-for="item in radio_items"
                    :key="item.id" :disabled="item.id!=2"
                    :label="item.id"
    ...
    methods: {
      init(){
        ...
        _this.ajax_api('get',url_api + '/device/region',null,true,function (res) {
          ...
          _this.ajax_api('get',url_api + '/inspect-type-choose/default-checks?inspectId=2',
            null,true,
            function (res) {
              ...
              for(let i=0;i<result_choose.length;i++){
                //choose_id_arr.push({id:result_choose[i].chooseId})
